...
import PaginationConsole from "../components/PaginationConsole";

function ConsoleImagePage() {
  const { id } = useParams();
  const [consoleDetail, setConsoleDetail] = useState({});
  const getConsoleList = `https://api.rawg.io/api/games?key=9165d834ffc64009b09c43f0a1ed0f67&page=1&page_size=24&platforms=${id}`;

  useEffect(() => {
    fetch(getConsoleList)
      .then((response) => response.json())
      .then((data) => setConsoleDetail(data));
  }, []);
  ...
  return (
    <>
      ...
      <div className="bg-white dark:bg-slate-900 py-6 sm:py-8 lg:py-12">
        <h2 className="text-emerald-800 dark:text-indigo-600 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-6"></h2>
        <PaginationConsole
          consoleDetail={consoleDetail}
          setConsoleDetail={setConsoleDetail}
        />
        <div className="max-w-screen-2xl px-4 md:px-8 mx-auto">
          <div className="flex justify-between items-end gap-4 mb-6">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 md:gap-x-6 gap-y-8">
              {/* .map() */}
              {consoleDetail?.results?.map((r) => {
                return (
                  <div
                    key={r.id}
...
export default ConsoleImagePage;
